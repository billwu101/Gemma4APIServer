...
.\install-tail-window.ps1          # Startup 放捷徑，登入時自動開 tail 視窗（不需管理員）
```

- Python 版本由 `.python-version` 釘在 3.12（pyenv）。`uvicorn[standard]` 的相依在 3.12 有現成
  wheel，3.13 需自行編譯。
- Docker 部署見 `docker-compose.yml`（閘道器 + Ollama 兩服務）：`docker compose up -d --build`，
  首次 `docker compose exec ollama ollama pull gemma4:26b`。容器內 uvicorn 一律綁 `0.0.0.0`
  （`.env` 的 `HOST` 不生效），閘道器經 `OLLAMA_BASE_URL=http://ollama:11434` 連容器版 Ollama；
  `.env` 靠 `env_file` 注入、不進 image。GPU 需 NVIDIA Container Toolkit，且會與本機 Ollama 搶 VRAM。
...
- 閘道器跑在 session 0，**沒有可見視窗**。觀察靠 `install-tail-window.ps1` 在 Startup 放的捷徑，
  登入時自動開一個彩色 tail 視窗（跑 `tail-log.ps1`）；關掉它不影響 server。
